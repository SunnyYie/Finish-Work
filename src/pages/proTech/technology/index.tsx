...
function Technology() {
  ...
  const handleMenuChange = (key) => {
    ...
    axios.get(`/api/technology?_id=${key}`).then((res) => {
      setFetchDataSource(res.data.data[0])
      setSelectKey(key)
      setLoading(false)

      // 请求关联数据
      axios.get(`/api/patent?technologyId=${res.data.data[0]._id}`).then((res) => {
        setRelationData(res.data.data)
        setRelationLoading(false)
      })
      axios.get(`/api/proposal?technologyId=${res.data.data[0]._id}`).then((res) => {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      setRelationLoading(true)
      // 请求产品数据
      axios.get(`/api/technology/${JSON.parse(value)._id}`).then((res) => {
    ...
    async function fetchData() {
      ...
      axios.get(`/api/technology/${JSON.parse(value)._id}`).then((res) => {
        if (selectKey !== '') {
          ...
          // 请求关联数据
          axios.get(`/api/patent?technologyId=${res.data.data.find((item) => item._id === selectKey)._id}`).then((res) => {
            setRelationData(res.data.data)
            setRelationLoading(false)
          })
          axios.get(`/api/proposal?technologyId=${res.data.data.find((item) => item._id === selectKey)._id}`).then((res) => {
            setRelationProposalData(res.data.data)
            setRelationLoading(false)
          })
        }
              ...
              return (
                <Menu.Item key={item._id as any}>
                  ...
                  <Button
                    type="text"
                    onClick={() => {
                      axios.delete(`/api/technology/${item._id}`).then(() => {
