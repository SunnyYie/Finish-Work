...
import { Drawer, Input, Tabs, Table, Tag, Space, Button } from '@arco-design/web-react'
...
export default function DrawerProduct(props) {
  ...
  const handleCreateProductPatent = () => {
    if (tabs === '1') {
      axios.put('/api/patent', { patentIds: selectedRowKeys, productId: productId }).then((res) => {
        setVisible(false)
        setSelectedRowKeys([])
        props.setUpdated((prev) => !prev)
      })
    } else {
      axios.put('/api/proposal', { proposalIds: selectProposalKeys, productId: productId }).then((res) => {
        setVisible(false)
